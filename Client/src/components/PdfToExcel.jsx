...
const PdfToExcel = () => {
  ...
  const [excelBlobUrl, setExcelBlobUrl] = useState(null); // State to store the downloadable URL

  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setSelectedFile(file);
      setExcelBlobUrl(null); // Reset URL if a new file is selected
    }
  };
  ...
  const handleConvertClick = async () => {
    ...
    setExcelBlobUrl(null); // Clear previous download URL
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/convert-pdf-to-excel',
        formData,
        {
  ...
  const handleDownloadClick = () => {
    ...
    a.href = excelBlobUrl;
    // Ensure the downloaded file name replaces .pdf with .xlsx
    a.download = selectedFile.name.replace(/\.pdf$/, '.xlsx');
    document.body.appendChild(a);
    a.click();
    a.remove();
    // Optionally, revoke the URL after download if you don't need it anymore.
    // window.URL.revokeObjectURL(excelBlobUrl);
  };

  const handleNewConversion = () => {
    // navigate('/pdf-to-excel'); // Use this if you have a specific route for the conversion page
    setSelectedFile(null);
    setIsConverting(false);
    setExcelBlobUrl(null); // Clear the download URL for a new conversion
  };

  // Render logic based on states
  // 1. If currently converting (showing spinner)
  if (isConverting) {
    return (
  ...
  // 2. If conversion is successful and a downloadable URL is available (show download screen)
  if (excelBlobUrl) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white p-6">
        <div className="text-center bg-white p-10 rounded-3xl shadow-xl border border-gray-200 max-w-md w-full">
          ...
          <p className="text-gray-700 mb-6">Your Excel file is ready to download.</p>
          <button
  ...
  // 3. Default state (initial file upload/conversion screen)
  return (
    <div className="min-h-screen flex items-center justify-center bg-white p-6">
      <div className="max-w-xl w-full bg-green-300 text-center rounded-3xl border border-white shadow-xl p-10">
        ...
        <p className="text-gray-700 mb-6">Easily convert your PDF files to Excel spreadsheets.</p>
        ...
        {selectedFile && (
          <div className="mt-4 p-3 bg-green-100 rounded-md">
            <p className="text-gray-700 font-medium flex items-center justify-center gap-2">
              <i className="fa-solid fa-file-pdf text-green-500"></i>
              {selectedFile.name}
            </p>
